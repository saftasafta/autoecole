...
import api from '../api';
import { FiPlus, FiTruck, FiTool, FiActivity, FiSearch, FiAlertTriangle, FiCheckCircle, FiShield, FiX, FiEdit, FiTrash2, FiClock } from 'react-icons/fi';
                  ...
                  <div style={{ marginTop:'2rem' }}>
                    ...
                    <div className="data-table-container">
                      <table className="data-table">
                        ...
                        <tbody>
                          {expenses.filter(ex => ['Repair', 'Insurance'].includes(ex.category) || ex.description?.includes('زيت') || ex.description?.includes('تأمين') || ex.description?.includes('فحص')).slice(0, 5).map(ex => (
                            <tr key={ex.id}>
                              ...
                              <td>
                                <div style={{ display:'flex', gap:'0.4rem' }}>
                                  <button className="btn btn-secondary" style={{ padding:'0.35rem 0.6rem' }} onClick={() => {
                                    const newDesc = window.prompt(t('description') + ':', ex.description || '');
                                    const newAmt  = window.prompt(t('amount') + ':', ex.amount);
                                    if (newAmt !== null) {
                                      api.put(`/api/vehicles/any/expenses/${ex.id}`, { ...ex, description: newDesc ?? ex.description, amount: newAmt })
                                        .then(() => fetchVehicleDetails(selectedVehicle.id));
                                    }
                                  }}><FiEdit size={13}/></button>
                                  <button className="btn" style={{ padding:'0.35rem 0.6rem', backgroundColor:'#FEE2E2', color:'#EF4444', border:'none' }}
                                    onClick={() => handleDeleteExpense(ex.id, 'Fleet')}>
                                    <FiTrash2 size={13}/>
                                  </button>
                                </div>
                              </td>
